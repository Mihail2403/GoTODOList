...
func NewTodoListPostgresRepo(db *sqlx.DB) *TodoListPostgres {
...
func (r *TodoListPostgres) Update(userId, listId int, input entity.UpdateTodoListInput) error {
	...
	query := fmt.Sprintf(`
		UPDATE %s tl SET %s 
		FROM %s ul 
		WHERE tl.id = ul.list_id AND ul.list_id = $%d AND ul.user_id=$%d`, todoListsTable, setQuery, usersListsTable, argId, argId+1)
